        ...
        try {
            ...
            if ($payment->isPaid() && !$payment->hasRefunds()) {
                ...
                if ($application->transaction_status != $payment->status) {
                    ...
                    $mutations = [];
                    $mutations[] = new cMutationRow($transaction->transaction_amount, GeneralLedgerAccount::ContributionMembersIncomes);

                    $mutationSold = new Mutation($mutations, PaymentMethod::Mollie, TransactionType::Received, "Contributie betaald");
